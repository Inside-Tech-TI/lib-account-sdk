export interface I2faInfo<T> {
  active: boolean;
  secret: string;
  mode: string; //notification-email, notification-sms, notification-whatsapp, authenticator-app
  data?: T; //for each mode can have different data
}
...
export interface IUserAccountInfo<T> {
  ...
  apps: { [appName: string]: UserContexts<any> };
}
